...
# Computing Activation Zone Boundaries for [SOTA](https://www.sota.org.uk/) Summits in W7W-SK 

LIDAR coverage is patchy for the Skagit region (SK), with no data for these summits: 
...
Import a GeoJSON file containing the locations and elevations of SOTA summits. I got these data from the SOTA APIs

```{r}
# get all summits via the SOTA API (because the sotlas GeoJSON doesn't have elevation data for summits in this region)

library(rvest)
gjsf <-  httr::GET("https://api2.sota.org.uk/api/regions/W7W/SK") %>% 
...
file_name <- paste0("W7W_SK/output/", str_replace_all(this_summit$summitCode, "/|-", "_"), 
                    ".geojson")
...
which_summit <- 
  which(summit_geometry$summit == "W7W_SK_181")
