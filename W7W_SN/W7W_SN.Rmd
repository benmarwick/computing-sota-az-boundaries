...
# Computing Activation Zone Boundaries for [SOTA](https://www.sota.org.uk/) Summits in W7W-SN
...
buffer_side_length <- 1e8 # 100 is a 10x10, this is the value we manually adjust to manage buffer area, to capture the biggest AZ
gjsf_elev_buf <- 
  bSquare(gjsf_elev,
          buffer_side_length)
...
SN-133 what's going on here? -> fixed
SN-028 too many polys -> fixed
SN-051 too big -> fixed
SN-076 too many polys -> wont fix, no lidar data
SN-078 too big -> fixed
SN-087 too big -> wont fix, no lidar data
SN-094 too big -> fixed
SN-095 too big -> wont fix, no lidar data
SN-113 too big  -> fixed
SN-119 too many poly  -> fixed
SN-123 too many poly -> fixed
SN-125 too big  -> fixed
SN-137 too big -> fixed
SN-139 too big -> fixed
SN-143 too big -> fixed
SN-146 too big -> fixed
SN-157 too big -> fixed
SN-173 SE cnr -> fixed
SN-178 SE cnr -> fixed
SN-183 too big -> fixed
SN-185 too big -> fixed

if not in poly, find nearest single poly
...
# n = 184
for(i in 130:nrow(gjsf_elev)){
  
 #  i <- which(gjsf_elev$summitCode == "W7W/SN-185")
...
# change this to 'nearest' polygon, because sometimes there are multiple
...
which_summit <- 
  which(summit_geometry$summit == "W7W_SN_185")
