...
The goal of these project is generate activation zone polygons as GeoJSON files that can be integrated into the sotl.as map for W7W. This is inspired by the work done for HB/HB0 and OE, where operators can view the activation zone polygon directly in the sotl.as map. Currently this project has computed polygons for activation zones for `r length(list.files(pattern = ".geojson", recursive = TRUE))` summits in the W7W area. Missing polygons are due to limitations in the coverage of the lidar data. 
...
And I have generated GeoJSON files for activation zones for these regions but I haven't yet sent to sotl.as:
