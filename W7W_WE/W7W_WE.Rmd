...
# Computing Activation Zone Boundaries for [SOTA](https://www.sota.org.uk/) Summits in W7W-WE
...
library(rvest)
gjsf <-  httr::GET(paste0("https://api2.sota.org.uk/api/regions/W7W/",
                          this_region)) %>% 
...
str_remove_all(x, "h") %>% 
  ...
  unlist() %>% 
  parse_number() %>% 
  -1 %>% # these are the IDs for wadnr_hillshade, 
  # looks like the DTM is simply 1 less
  str_c( collapse = "%2C") 
```
...
buffer_side_length <- 5e7 # 100 is a 10x10, this is the value we manually adjust to manage buffer area, to capture the biggest AZ
gjsf_elev_buf <- 
  bSquare(gjsf_elev,
          buffer_side_length)
...
# this is for checking the quality of a small set of AZs that look odd
# need to check
check <- c(
"021",  # too square -> done
"037",  # too square -> done
"043",  # sw and ne cnr -> done
"044",  # sw and ne cnr  -> done
"045",  # w edge -> done
"046",  # w edge -> done
"047",  # three sides -> done
"048",  # w and e edges -> done
"052",  # jagged -> done
"053",  # w edge -> done
"054",  # w edge -> done
"058",  # all sides -> done
...
j <- which(str_detect(gjsf_elev$summitCode, paste0(check, collapse = "|")))
```
...
for(i in 1:length(res_n)){
  ...
  if(sum(rasters_res_tbl$res == the_res) == 1){
    ...
    # if multiple raster files of that resolution
      ...
      print(paste0("Merging LIDAR raster files with the same resolution for ",
                   this_summit$summitCode,
             "..."))
    
    list_of_rasters[[i]]  <-  
      sprc(rasters_res_tbl$file[ rasters_res_tbl$res == the_res ])
    list_of_rasters[[i]]  <- 
      terra::merge(list_of_rasters[[i]], 
                   gdal=c("BIGTIFF=YES", 
...
this_summit_point_az <- 
  ...
  mutate(lidar_cropped_max_elev_ft = lidar_cropped_max_elev_ft, 
         ...
         az_lower_contour = ifelse(elev_m <= lidar_cropped_max_elev_m,
                                   elev_m - az_elev_m,         
                                   lidar_cropped_max_elev_m - az_elev_m ),  
         # SOTA summit data does not always match raster data
...
az_poly <- sf::st_as_sf(terra::as.polygons(lidar_cropped > -Inf))
...
poly_with_summit <- 
  apply(st_is_within_distance(df_union_cast, 
                              this_summit_nad83, 
                              sparse = FALSE,
                              dist = 5), 2, # within or X m outside of,
        # because some summits are just outside of their nearest polygon
        function(col) { 
          df_union_cast[which(col), ]
        })[[1]]

# sometimes they have holes in them, let's remove those holes
# poly_with_summit <- sfheaders::sf_remove_holes(poly_with_summit)
...
which_summit <- 
  which(summit_geometry$summit == "W7W_MC_001")
...
Panel of static plots to inspect them in batches
...
# inspect a batch
n <- 50:60
list_plots <- vector("list", length = (max(n) - n[1]))
for(i in 1:length(n)){
 ...
 annotation_scale(location = "bl",
              ...
              pad_x = unit(0.5, "cm"),
              style =  "ticks") +
   ggtitle(label = summit_geometry[n[i],]$summit) +
   theme_minimal(base_size = 3) +
   theme(title = element_text(size = 10)) 
 
 names(list_plots)[i] <-  summit_geometry[n[i],]$summit
}
