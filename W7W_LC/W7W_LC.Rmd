...
# Computing Activation Zone Boundaries for [SOTA](https://www.sota.org.uk/) Summits in W7W-LC 
...
library(rvest)
gjsf <-  GET("https://api2.sota.org.uk/api/regions/W7W/LC") %>% 
            content(., as = "text")%>% 
            jsonlite::fromJSON() %>% 
            purrr::pluck("summits") %>% 
            tibble() %>% 
...
file_name <- paste0("W7W_LC/output/", str_replace_all(this_summit$summitCode, "/|-", "_"), 
                    ".geojson")
...
gjsf <- st_read("W7W_LC/input/W7W_LC.geojson")

which_summit <- 
  which(summit_geometry$summit == "W7W_LC_153")
